...
```powershell
Compress-Archive -Path manifest.json, _locales, icons, src, LICENSE -DestinationPath dist/sitelemetry-audit-0.1.2.zip -CompressionLevel Optimal
```
...
[System.IO.Compression.ZipFile]::OpenRead("dist/sitelemetry-audit-0.1.2.zip").Entries.FullName
```

On PowerShell 7 (`pwsh`) or with the built-in `tar` (`tar -a -c -f dist/sitelemetry-audit-0.1.2.zip manifest.json _locales icons src LICENSE`) the names are already correct.
